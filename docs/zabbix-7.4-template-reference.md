...
- ✅ **VERIFIED (7.4):** **`configuration.import` never DELETES unless `deleteMissing: true`** for that entity in the rules. Removing an item/trigger from a template YAML and re-importing does NOT remove it from the server — it lingers as an orphan (bit us: a removed "No data received" trigger kept firing after re-import). To make the YAML authoritative, set `deleteMissing: true` for `items`/`triggers`/`templateDashboards`. Trade-off: hand-added objects in the UI get deleted on the next import — so manage those templates via the repo, not the UI.
...
- **MQTT liveness ≠ HTTP liveness.** Over MQTT, use the **`online` (LWT)** topic with `last(online)=0`, NOT `nodata()` on a status topic. Shelly publishes status topics at very different cadences — `pm1:0`/`switch:0` update often, but **`sys` can be 10-15+ min stale while the device is perfectly alive**. A `nodata(sys,10m)` liveness trigger therefore false-fires constantly. The LWT topic reflects the actual MQTT session (broker sets it false on disconnect), independent of publish cadence — so it's the correct, cadence-proof liveness signal. (HTTP has no LWT, hence the nodata approach there.)
